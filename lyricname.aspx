<%@ Page Language="C#" MasterPageFile="~/SiteTemplate.master" EnableViewState="false" AutoEventWireup="true" CodeFile="lyricname.aspx.cs" Inherits="lyricalphaletter" Title="Untitled Page" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
    ...
    <option value="lyricname.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=<%=psPageSize%>&layout=1" >Rows</option>
    <option value="lyricname.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=<%=psPageSize%>&layout=2">Grid 2 Columns</option>
    <option value="lyricname.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=<%=psPageSize%>&layout=3">Grid 3 Columns</option>
    </select>
    <select name="url" class="ddlps" onchange="javascript:document.location = options[selectedIndex].value">
    <option selected>
    <%=psPageSize + " Records Page"%>
    </option>
    <option value="lyricname.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=10&layout=<%=pLayout%>">Hiển thị 10 món</option>
    <option value="lyricname.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=20&layout=<%=pLayout%>">Hiển thị 20 món</option>
    <option value="lyricname.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=30&layout=<%=pLayout%>">Hiển thị 30 món</option>
    <option value="lyricname.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=40&layout=<%=pLayout%>">Hiển thị 40 món</option>
    <option value="lyricname.aspx?letter=<%=psLetter%>&ob=<%=psOrderBy%>&sb=<%=psSortBy%>&page=<%=psPageIndex%>&ps=50&layout=<%=pLayout%>">Hiển thị 50 món</option>
    </select>
    </div>
    ...
    <asp:DataList id="LyricCat" OnItemDataBound="LyricCatItemDataBound" RepeatDirection="Horizontal" runat="server" EnableViewState="false">
          <ItemTemplate>
        <div class="divwrap5">
           <div class="divhd">
   ...
   Category:&nbsp;<a class="dt2" title="Go to <%# Eval("Category") %> category" href='<%# Eval("CatID", "category.aspx?catid={0}") %>'><%# Eval("Category") %></a>
    <br />
    By:&nbsp;<img src="images/user-icon.gif" />&nbsp;<a href="findalllyricbyauthor.aspx?author=<%# Eval("Author") %>" onmouseover="Tip('View all recipe submitted by <b><%# Eval("Author") %></b>.', BGCOLOR, '#FFFBE1', BORDERCOLOR, '#acc6db')" onmouseout="UnTip()"><%# Eval("Author") %></a>
    <br />
